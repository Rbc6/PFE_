...
export class AddParamedicalComponent implements OnInit {
  ...
  constructor(public dialogRef : MatDialogRef<AddParamedicalComponent>,
    private formBuilder : FormBuilder,
    private parametresService : ParametresService) { }
  Form!: FormGroup;
  ngOnInit(): void {  this.Form = this.formBuilder.group({
    paraMed :['',Validators.required]
  ...
  OnSubmit(){
    if (this.Form.valid) {
      ...
      this.parametresService.createParamedical(this.Form.value).subscribe(res => {
        Swal.fire({
          title: 'Succès !',
          text: 'La paramédical a été ajouté avec succès.',
          icon: 'success',
          timer: 4000,
